...
class MetricTemporalBEVFusion:
    """Short-term metric observation memory in ``temporal_frame``.

    Unlike image/grid warping, each observed BEV cell is stored as an x/y
    observation in odom (or another fixed frame).  On every update those
    metric observations are re-rasterized into the *current* base_link grid.
    A head turn therefore cannot erase free space that was observed moments
    earlier merely because the camera field of view changed.
    """
    ...
    def update(self, raw: BEVGrid, now_s: float,
               T_temporal_from_current: Optional[np.ndarray]) -> BEVGrid:
        if not self.cfg.enabled or T_temporal_from_current is None:
            self.reset()
            return raw
        ttl = max(0.1, 3.0 * float(self.cfg.evidence_half_life_s))
        self._chunks = [c for c in self._chunks if now_s - c[0] <= ttl]

        observed = raw.observed
        ii, jj = np.nonzero(observed)
        if ii.size:
            x = raw.cfg.x_min + (ii.astype(np.float32) + .5) * raw.cfg.resolution
            y = raw.cfg.y_min + (jj.astype(np.float32) + .5) * raw.cfg.resolution
            R, t = T_temporal_from_current[:2, :2], T_temporal_from_current[:2, 3]
            xy = np.column_stack((x, y)) @ R.T + t
            self._chunks.append((now_s, xy.astype(np.float32),
                                 raw.traversability[ii, jj].astype(np.float32),
                                 raw.obstacle[ii, jj].astype(np.float32)))

        shape = raw.shape
        obs_weight = np.zeros(shape, np.float32)
        trav_sum = np.zeros(shape, np.float32)
        obstacle_score = np.zeros(shape, np.float32)
        T_current_from_temporal = np.linalg.inv(T_temporal_from_current)
        R, t = T_current_from_temporal[:2, :2], T_current_from_temporal[:2, 3]
        half_life = max(float(self.cfg.evidence_half_life_s), 1e-3)
        for stamp, xy, trav, obst in self._chunks:
            xy_current = xy @ R.T + t
            i = ((xy_current[:, 0] - raw.cfg.x_min) / raw.cfg.resolution).astype(np.int32)
            j = ((xy_current[:, 1] - raw.cfg.y_min) / raw.cfg.resolution).astype(np.int32)
            inside = (i >= 0) & (i < shape[0]) & (j >= 0) & (j < shape[1])
            if not inside.any():
                continue
            weight = float(np.exp(-(now_s - stamp) * np.log(2.0) / half_life))
            i, j = i[inside], j[inside]
            np.add.at(obs_weight, (i, j), weight * self.cfg.observation_hit)
            np.add.at(trav_sum, (i, j), weight * self.cfg.observation_hit * trav[inside])
            np.add.at(obstacle_score, (i, j), weight * self.cfg.obstacle_hit * obst[inside])

        observed_out = obs_weight >= self.cfg.min_observed_evidence
        traversability = np.zeros(shape, np.float32)
        np.divide(trav_sum, np.maximum(obs_weight, 1e-6), out=traversability,
                  where=obs_weight > 0.0)
        obstacle_confidence = obstacle_score / np.maximum(obs_weight, 1e-6)
        obstacle = obstacle_confidence >= self.cfg.obstacle_evidence_threshold
        # A fresh free observation is represented by a stronger/newer sample
        # and naturally lowers old obstacle evidence below the threshold.
        observed_out[obstacle] = True
        traversability[obstacle] = 0.0

        xs = raw.cfg.x_min + (np.arange(shape[0], dtype=np.float32) + .5) * raw.cfg.resolution
        ys = raw.cfg.y_min + (np.arange(shape[1], dtype=np.float32) + .5) * raw.cfg.resolution
        xg, yg = np.meshgrid(xs, ys, indexing="ij")
        ego = (xg * xg + yg * yg) <= self.cfg.ego_clear_radius_m ** 2
        fill = ego & ~observed_out & ~obstacle
        observed_out[fill] = True
        traversability[fill] = self.cfg.ego_clear_traversability
        counts = np.rint(obs_weight).astype(np.int32)
        return BEVGrid(traversability, observed_out, obstacle, counts,
                       np.rint(trav_sum).astype(np.int32),
                       obstacle_count=np.rint(obstacle_score).astype(np.int32),
                       cfg=raw.cfg, ground_z=raw.ground_z,
                       obstacle_pixels=raw.obstacle_pixels)
